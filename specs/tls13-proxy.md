...
| 9 | Fixtures + tests + bench + version flip | `server-fixture`, `harness`, `tlsn` | §9.1; spec `specs/tasks/tls13-e2e-fixture-bench.md`; depends on item 8b (DONE) — **unblocked**. Owns the client version-list flip to `&[&TLS13,&TLS12]`, a version-configurable fixture (`bind_with_versions`), the live prover↔verifier 1.3 e2e + negotiation matrix, the webpki cert-chain happy path (deferred from item 5), and the dual-allocation cost bench (open-question §2). The first live end-to-end 1.3 run. |
